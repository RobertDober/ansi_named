defmodule Test.Ansi.AnsiCodeTest do
  use ExUnit.Case

  import ColorNamed, only: [ansi_code: 1, ansi_color: 1]

  describe "legal ansi codes" do
    definitions =
    %{ 
      "black" => 0,
      "maroon" => 1,
    ...
    fn {name, ansi_code_number} ->
      tag = String.to_atom(name)
      quote do
        unquote do
          @tag tag
          test("color: '" <> name <> "' (#{ansi_code_number})") do
            assert ansi_color(unquote(name)) == {unquote(ansi_code_number)}
          end
        end
      end
    end)
    ...
    fn {name, ansi_code_number} ->
      ...
      quote do
        unquote do
          @tag tag
          test("code: '" <> name <> "' (#{ansi_code_number})") do
            assert ansi_code(unquote(name)) == "\e[38;5;#{unquote(ansi_code_number)}m"
          end
        end
      end
    end)
